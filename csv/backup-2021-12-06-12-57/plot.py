...
def run(start, end, flag):
    ...
    if True:
       ...
       pairs2 = [
        ...
        ['fedtrpo-nonlinear-figureeightv1-5iter-tv-1e-4-1e-1-iid-ec.csv', 'FedTRPO-nonlinear-figureeightv1-5iter-1e-4-1e-1-iid-ec', 'dodgerblue', '-'],
        # ['fedtrpo-nonlinear-figureeightv1-5iter-tv-1e-4-2e-2-iid-ec.csv', 'FedTRPO-nonlinear-figureeightv1-5iter-1e-4-2e-2-iid-ec', 'turquoise', '-'],
       ]
       pairs = pairs2
       handles, labels = [], []
       for pair in pairs:
           if 'initstate' not in pair[0] or '10iter' not in pair[0]:
               # continue
               pass
